...
> **Current status of that question.** The two controllers provably receive the **same per-step QP** (bit-identical arrays from one shared builder, identical prediction model). On the closed loop their applied controls agree to **0.707 °C RMS**, every step is feasible, and **every applied move is now produced by the solver** — the downstream safety clip corrects **0.0 %** of them, down from 12.9 % in the stiff window before the projection budget was repaired. The SNN's formal convergence criterion is met on **50.0 %** of nominal steps, but only **16.1 %** inside the stiff exotherm window, and **0 %** at N=20 where the certificate fails by 113×. The honest verdict is **"same QP; feasible everywhere, every applied move solver-produced, but convergence is certified on only a minority of steps and least often where the controller matters most"** — *not* equivalence. The full write-up is **[`docs/SNN_MPC_TECHNICAL_REPORT.pdf`](docs/SNN_MPC_TECHNICAL_REPORT.pdf)** (Revision 3); the step-by-step evidence, definitions and limitations behind it are in **[`docs/PHASE4_VALIDATION_REPORT.md`](docs/PHASE4_VALIDATION_REPORT.md)** (Revision 5), and every reported figure is mapped to its source file in [`results/artifact-index.md`](results/artifact-index.md).
>
...
- [x] **Phase 4: Neuromorphic SNN Integration**
  ...
  - Ran on the compiled `snn_opt` kernel (`backend='c'`): ~85× faster than the pure-Python reference path. (An earlier version of this line called the timing "comparable to the OSQP baseline"; that claim was withdrawn — see the compute-time caveat below.)
...
- [x] **Phase 5: Closing the stiff window** (Revision 5) — see [`docs/PHASE4_VALIDATION_REPORT.md`](docs/PHASE4_VALIDATION_REPORT.md) §14–§18
...
- [x] **Phase 6: Advisor review** — see [`docs/PHASE4_VALIDATION_REPORT.md`](docs/PHASE4_VALIDATION_REPORT.md) §19
  - Convergence rate is confirmed as a diagnostic, not a blocker: 0 % clipping with every applied move solver-produced is an acceptable software-phase stopping point regardless of the stiff-window rate, since a genuinely infeasible step fails any solver's stopping test by construction.
  - The frozen-Jacobian over-prediction (§14) and the stiff-step infeasibility are confirmed as **the same mechanism** by direct measurement: driving the frozen model and a true nonlinear rollout with the identical control sequence at six stiff states shows **63.3 % of currently-flagged gradient-row violations are artifacts** of the frozen prediction, with zero missed. The LTV rewrite is recommended as the next step, scoped as a feasibility repair — not a convergence one.
  - Contribution scope settled: this work's claim is solver equivalence under a neuromorphic-hardware-efficiency motivation, not controller performance; the prediction-model limitation is stated, not pursued further here.
...
The SNN-QP is handed the **identical** per-step QP as the baseline (verified bit-identical, `tests/test_qp_parity.py`), and reaches full uniform cure with an overshoot of 13.23 °C against the baseline's 13.77 °C. That closeness is a measured agreement of the *applied control* (0.707 °C RMS), **not** a demonstration that the solver converged — its formal criterion is met on 50.0 % of nominal steps but only 16.1 % in the stiff exotherm window. Every applied move is now produced by the solver — the downstream safety clip corrects 0.0 % of them, down from 12.9 % in the stiff window before the projection budget was repaired. See [`docs/PHASE4_VALIDATION_REPORT.md`](docs/PHASE4_VALIDATION_REPORT.md) for what is and is not established.
