...
Diagnostic-only harness implementing the `closed-loop-equivalence` skill: one
loop, two plants, the SAME disturbance-injection point (disturbance-before-
compute, per the skill's prescribed pattern) for both controllers, so the two
...
def main():
    ...
    manifest = {
        "convention": "disturbance-before-compute (closed-loop-equivalence skill pattern)",
        "note_on_fingerprints": (
            ...
            "controller canonical form (see qp-contract skill's 'Required output' for "
