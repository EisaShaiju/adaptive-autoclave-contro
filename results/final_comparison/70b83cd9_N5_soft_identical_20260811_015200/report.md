...
Generated 2026-08-11T01:51:56 from git commit `70b83cd9b5dd35b2caf967e936355102037f597b` (DIRTY -- see provenance in summary.json).

## Method

One shared harness (`tools/final_controlled_comparison.py`) drives two independent `AutoclavePlant` instances, one per controller, from the identical initial state (28.0 degC), the same target temperature (120.0 degC), the same horizon (N=20), the same sampling time (TE=60.0s), the same physical constraints, and the same canonical per-step QP construction (`src/qp_builder.py`) -- the only permitted divergence is the `trust_region` flag baked into each controller's own `Ap,Bp` (documented). Disturbance convention: **disturbance-before-compute**, applied identically to both plants at the same step index.
...
**Scenario 3 (stiff exotherm) is a window, not a separate simulation**: steps [0, 25) of `nominal_heatup`, centered on its peak `rho(Ap)` (k=4, rho=1.0000) -- the gelation region both controllers pass through in the standard 160-step run. No separate plant initial condition was introduced (would have required modifying `AutoclavePlant`, out of scope).

## Heat-up slew-saturation caveat

Of the first 60 steps (heat-up phase, before any disturbance), **3 steps (5.0%)** had BOTH controllers pinned at the `TA_RATE_MAX` slew limit (4.0 degC/min) simultaneously.

This is a small fraction of the heat-up phase -- slew saturation is not a major confound for the heat-up-phase agreement reported below, but is reported for completeness and to avoid silently omitting the check.
...
|---|---|---|---|---|---|---|---|---|
| nominal_heatup | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 98.75% | 6.5332e-07 | 0 (0.0%) | -0.0000 |
| disturbance_step60 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 98.75% | 6.5332e-07 | 0 (0.0%) | -0.0000 |
| stiff_exotherm_window (steps 0-24 of nominal_heatup) | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 92.00% | 6.5332e-07 | 0 (0.0%) | -0.0000 |
...
- nominal_heatup: 160/160 steps (100.0%) were feasible (scaled) enough to compute an objective gap; verified SNN convergence rate 98.75% (raw self-reported `converged` rate 98.75%).
- disturbance_step60: 160/160 steps (100.0%) feasible; verified convergence rate 98.75%.
- stiff_exotherm_window: 25/25 steps (100.0%) feasible; verified convergence rate 92.00%; max constraint residual 6.5332e-07 -- consistent with the earlier finding that both solvers can struggle on the raw scaled arrays at the gelation peak.
