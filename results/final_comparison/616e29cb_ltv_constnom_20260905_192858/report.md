...
Generated 2026-09-05T19:27:50 from git commit `616e29cbf12bab7a7476229c7464a38296d0feea` (DIRTY -- see provenance in summary.json).
...
**Scenario 3 (stiff exotherm) is a window, not a separate simulation**: steps [78, 109) of `nominal_heatup`, centered on its peak `rho(Ap)` (k=88, rho=1.4823) -- the gelation region both controllers pass through in the standard 160-step run. No separate plant initial condition was introduced (would have required modifying `AutoclavePlant`, out of scope).
...
|---|---|---|---|---|---|---|---|---|
| nominal_heatup | 1.0568 | 0.2267 | 3.5878 | 0.6870 | 56.25% | 7.6158e-07 | 0 (0.0%) | 0.0004 |
| disturbance_step60 | 1.8872 | 0.4252 | 7.2078 | 1.2109 | 49.38% | 7.6158e-07 | 0 (0.0%) | 0.0005 |
| stiff_exotherm_window (steps 78-108 of nominal_heatup) | 2.0727 | 0.3895 | 3.5878 | 0.6861 | 41.94% | 7.2512e-07 | 0 (0.0%) | 0.0007 |
...
- nominal_heatup: 160/160 steps (100.0%) were feasible (scaled) enough to compute an objective gap; verified SNN convergence rate 56.25% (raw self-reported `converged` rate 56.25%).
- disturbance_step60: 160/160 steps (100.0%) feasible; verified convergence rate 49.38%.
- stiff_exotherm_window: 31/31 steps (100.0%) feasible; verified convergence rate 41.94%; max constraint residual 7.2512e-07 -- consistent with the earlier finding that both solvers can struggle on the raw scaled arrays at the gelation peak.
